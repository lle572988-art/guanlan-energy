...
function getCompareArticleSchema(page, site = DEFAULT_SITE) {
  const pageUrl = absUrl(site, `/pages/${page.slug}.html`);
  const desc = page.description || page.meta_description || '';
  return {
    '@context': 'https://schema.org',
    '@graph': [
      ...
      getFAQSchema([
        {
          question: page.faq_q || page.faq_question || `How does this compare to other systems?`,
          answer: page.faq_a || page.faq_answer || desc,
        },
      ]),
      getBreadcrumbSchema(
        [
          { name: 'Home', url: '/' },
          { name: 'Compare', url: '/pages/' },
...
function getHowToReadChartSchema(site = DEFAULT_SITE) {
  const calcUrl = absUrl(site, site.cta_page || '/free-chart.html');
  const guideUrl = absUrl(site, '/pages/what-is-ming-gong-life-palace.html');
  return {
    '@context': 'https://schema.org',
    '@type': 'HowTo',
    name: 'How to Read a Zi Wei Dou Shu (ZWDS) Chart',
    description:
      'Step-by-step guide to understanding your Purple Star Astrology birth chart — 12 palaces, major stars, and four transformations.',
    totalTime: 'PT15M',
    tool: [{ '@type': 'HowToTool', name: 'Free ZWDS Calculator', url: calcUrl }],
    step: [
      {
        '@type': 'HowToStep',
        position: 1,
        name: 'Generate your birth chart',
        text: 'Enter birth date, time, and gender into a ZWDS calculator. Precision of birth hour strongly affects palace placement.',
        url: calcUrl,
      },
      {
        '@type': 'HowToStep',
        position: 2,
        name: 'Locate your Life Palace (Ming Gong)',
        text: 'The Life Palace reveals core personality and destiny tone — start every reading here.',
        url: guideUrl,
      },
      {
        '@type': 'HowToStep',
        position: 3,
        name: 'Read Career and Wealth palaces',
        text: 'Cross-reference Career (官祿) and Wealth (財帛) with Life Palace for practical direction.',
        url: calcUrl,
      },
    ],
  };
...
module.exports = {
  ...
  getCalculatorSchema,
  getCompareArticleSchema,
  getHowToReadChartSchema,
  getPageGraphSchema,
  toScriptTag,
};
